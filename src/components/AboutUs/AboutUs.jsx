...
const AboutUs = () => {
  return (
    <div className="p-4">
      <h2 className="text-3xl font-bold font-poppins text-moradito text-center mt-9 mb-9">Sobre Nosotros</h2>
      <p className="text-moradito text-lg text-justify mb-4 mt-4 font-poppins">
        Somos un equipo de desarrolladores apasionados por la tecnología y la creación de aplicaciones innovadoras. </p> 
      <p className="text-moradito text-lg text-justify mb-4 mt-4 font-poppins">Nuestra última creación es una aplicación de películas y series que combina la potencia de Express, MongoDB, Node.js, React, Redux, JavaScript, HTML y Tailwind CSS para ofrecer una experiencia única a los amantes del entretenimiento.
      </p>
      <h3 className="text-3xl font-bold text-moradito mb-5 font-poppins mt-9 text-center ">Conócenos:</h3>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
        {teamMembers.map((member, index) => (
          <div key={index} className="rounded-lg border p-4 text-center">
            ...
            />
            <p className="text-moradito font-poppins text-lg mb-2">{member.name}</p>
            <a href={member.githubURL} target="_blank" rel="noopener noreferrer" className="text-moradito font-poppins text-lg mb-2 hover:text-lila">
              <FaGithub className="mr-2 inline" /> GitHub
            </a>
            <a href={member.linkedinURL} target="_blank" rel="noopener noreferrer" className="text-moradito font-poppins text-lg hover:text-lila">
              <FaLinkedin className="mr-2 inline ml-10" /> LinkedIn
            </a>
          </div>
        ))}
      </div>
      <h3 className="text-3xl font-bold text-moradito mb-5 font-poppins mt-9 text-center ">Acerca de Did U Watch It?:</h3>
      <p className="text-moradito text-lg text-justify mb-4 font-poppins">
